...
type CreateMemberRequest struct {
	Nickname string   `json:"Nickname" form:"Nickname"` // required，不小于 4 位 不超过 20 位
	Username string   `json:"Username" form:"Username"` // required，只支持大小写，长度不小于 8 位 不超过 20 位
	Password string   `json:"Password" form:"Password"` // required，同时包括大小写、数字，长度不少于 8 位 不超过 20 位
	UserType UserType `json:"UserType" form:"UserType"` // required, 枚举值
}
...
type ScheduleCourseResponse struct {
	...
	// key 为 teacherID , val 为老师最终绑定的课程 courseID
}
